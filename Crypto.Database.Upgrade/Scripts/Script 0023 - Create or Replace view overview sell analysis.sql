...
AS SELECT e.name AS exchange_name,
    ...
    a.break_even AS isstoplossactive,
    a.price_movement,
   ...
   FROM positive_signal
     ...
     LEFT JOIN sell_analysis2 a ON positive_signal.signal_id = a.signal_id
...
-- GRANT ALL, SELECT, INSERT, UPDATE, DELETE, REFERENCES(current_band_entry_time) ON public.overview TO postgres;
